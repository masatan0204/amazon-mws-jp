...
module.exports = {
    ...
    GetServiceStatus: function (cb) {
        if(mws.checkConfig() == false){
          cb("config error", null);
          return;
        }
     ...
     * description : SellerSKUに応じた、商品の親カテゴリーを返します。
     *   GetProductCategoriesForSKUオペレーションは、マーケットプレイスのルートまでさかのぼった親カテゴリーも含め、商品が属するカテゴリーの名前とIDを返します。
     * see         : http://docs.developer.amazonservices.com/ja_JP/products/Products_GetProductCategoriesForSKU.html
     *
     * param params : SellerSKU'
     * param cb     : callback function
     * return       : MWS API result (xml)
     *
     * example of execuion. ("TM-201705-037"というSKUの商品を情報を取得する場合)
     ...
     * );
     */
    GetProductCategoriesForSKU: function (params, cb) {
        if(mws.checkConfig() == false){
          cb("config error", null);
          return;
        }
